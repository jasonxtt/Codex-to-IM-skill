...
describe('permission-broker - qq text permissions', () => {
  ...
  it('sends plain text prompt for qq (no buttons)', async () => {
    ...
    assert.ok(permMsg.text.includes('1 - 允许一次') || permMsg.text.includes('1 - Allow once'), 'Should contain shortcut 1');
    assert.ok(permMsg.text.includes('2 - 本会话允许') || permMsg.text.includes('2 - Allow session'), 'Should contain shortcut 2');
    assert.ok(permMsg.text.includes('3 - 拒绝') || permMsg.text.includes('3 - Deny'), 'Should contain shortcut 3');
...
describe('bridge-manager - image download failure reply', () => {
  ...
  it('replies to user when image-only message fails download', async () => {
    ...
    assert.ok(
      sentMessages[0].text.includes('下载 2 个image(s)失败') || sentMessages[0].text.includes('Failed to download 2 image(s)'),
    );
  ...
  it('replies to user when non-image attachments fail download', async () => {
    ...
    assert.equal(sentMessages.length, 1);
    assert.ok(
      sentMessages[0].text.includes('下载 1 个attachment(s)失败') || sentMessages[0].text.includes('Failed to download 1 attachment(s)'),
    );
...
describe('numeric shortcut permission replies', () => {
  ...
  it('resolves single pending permission with "1" → allow (qq)', async () => {
    ...
    assert.ok(sentMessages[0].text.includes('允许一次') || sentMessages[0].text.includes('Allow'), 'Should confirm Allow');
  ...
  it('resolves single pending permission with "2" → allow_session (qq)', async () => {
    ...
    assert.equal(sentMessages.length, 1);
    assert.ok(
      sentMessages[0].text.includes('本会话允许') || sentMessages[0].text.includes('Allow Session'),
      'Should confirm Allow Session',
    );
  ...
  it('resolves single pending permission with "3" → deny (qq)', async () => {
    ...
    assert.equal(sentMessages.length, 1);
    assert.ok(sentMessages[0].text.includes('拒绝') || sentMessages[0].text.includes('Deny'), 'Should confirm Deny');
  ...
  it('handles fullwidth digit ２ via NFKC normalization', async () => {
    ...
    assert.ok(
      sentMessages[0].text.includes('本会话允许') || sentMessages[0].text.includes('Allow Session'),
      'Fullwidth ２ should map to allow_session',
    );
  ...
  it('handles digit with zero-width characters', async () => {
    ...
    assert.ok(sentMessages[0].text.includes('允许一次') || sentMessages[0].text.includes('Allow'), 'Should map to allow');
  ...
  it('hints user to use /perm when multiple pending permissions', async () => {
    ...
    assert.ok(
      sentMessages[0].text.includes('待处理权限') || sentMessages[0].text.includes('Multiple pending permissions'),
      'Should mention multiple pending',
    );
  ...
  it('old /perm command still works alongside numeric shortcuts', async () => {
    ...
    assert.ok(
      sentMessages.some((m) => m.text.includes('已记录') || m.text.includes('recorded')),
      'Should confirm via /perm',
    );
  });
});

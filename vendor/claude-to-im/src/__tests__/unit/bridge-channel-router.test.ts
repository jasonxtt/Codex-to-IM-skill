...
function createMockStore(): BridgeStore & { bindings: Map<string, ChannelBinding>; sessions: Map<string, { id: string; working_directory: string; model: string; sdk_session_id?: string; createdAt?: string }> } {
  const bindings = new Map<string, ChannelBinding>();
  const sessions = new Map<string, { id: string; working_directory: string; model: string; sdk_session_id?: string; createdAt?: string }>();
  let nextId = 1;
  ...
  return {
    ...
    createSession(name: string, model: string, _systemPrompt?: string, cwd?: string) {
      const session = { id: `session-${nextId++}`, working_directory: cwd || '', model, createdAt: new Date().toISOString() };
...
describe('channel-router', () => {
  ...
  it('bindToSession() restores persisted sdk session id', () => {
    const session = store.createSession('Test', 'claude-3', undefined, '/test');
    session.sdk_session_id = 'sdk-thread-1';
    const binding = router.bindToSession(
      { channelType: 'telegram', chatId: '789' },
      session.id,
    );
    assert.equal(binding?.sdkSessionId, 'sdk-thread-1');
  });

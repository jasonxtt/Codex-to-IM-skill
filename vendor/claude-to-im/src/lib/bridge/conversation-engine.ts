...
const APPROVAL_POLICIES: ReadonlySet<StreamApprovalPolicy> = new Set([
  'never',
  'on-request',
  'on-failure',
  'untrusted',
...
const SANDBOX_MODES: ReadonlySet<StreamSandboxMode> = new Set([
  'read-only',
  'workspace-write',
  'danger-full-access',
]);
...
export async function processMessage(
  ...
  try {
    ...
    const stream = llm.streamChat({
      ...
      files,
      ...resolveRuntimeOptions(store, binding.permissionProfile),
...
function resolveRuntimeOptions(
  ...
  const configuredSandboxMode = parseSandboxMode(
    getFirstSetting(store, ['bridge_sandbox_mode', 'bridge_codex_sandbox_mode']),
  );
  const networkAccessEnabled = parseBooleanSetting(
    getFirstSetting(store, [
      'bridge_network_access_enabled',
      'bridge_network_access',
      'bridge_codex_network_access_enabled',
      'bridge_codex_network_access',
    ]),
  );
  const additionalDirectories = parseStringArraySetting(
    getFirstSetting(store, [
  ...
  return {
    approvalPolicy: profileOverrides.approvalPolicy ?? configuredApprovalPolicy,
...
function getFirstSetting(store: SettingsProvider, keys: string[]): string | null {
...
function parseApprovalPolicy(value: string | null): StreamApprovalPolicy | undefined {
  if (!value || !APPROVAL_POLICIES.has(value as StreamApprovalPolicy)) return undefined;
  return value as StreamApprovalPolicy;
}

function parseSandboxMode(value: string | null): StreamSandboxMode | undefined {
  if (!value || !SANDBOX_MODES.has(value as StreamSandboxMode)) return undefined;
  return value as StreamSandboxMode;
}

function parseBooleanSetting(value: string | null): boolean | undefined {
...
function parseStringArraySetting(value: string | null): string[] | undefined {
  if (!value) return undefined;

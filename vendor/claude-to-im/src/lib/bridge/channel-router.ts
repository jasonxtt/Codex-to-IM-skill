...
import type { ChannelAddress, ChannelBinding, ChannelType, PermissionProfile } from './types.js';
...
export function createBinding(
  ...
  const defaultPermissionProfile = resolveDefaultPermissionProfile();
...
export function bindToSession(
  ...
  return store.upsertChannelBinding({
    ...
    model: session.model,
    permissionProfile: resolveDefaultPermissionProfile(),
  });
}
...
export function updateBinding(
  id: string,
  updates: Partial<Pick<ChannelBinding, 'sdkSessionId' | 'workingDirectory' | 'model' | 'mode' | 'permissionProfile' | 'active'>>,
): void {
...
function resolveDefaultPermissionProfile(): PermissionProfile {
  const { store } = getBridgeContext();
  const raw = store.getSetting('bridge_default_permission_profile')?.trim().toLowerCase();

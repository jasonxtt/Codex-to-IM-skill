  ...
  private async registerCommands(): Promise<void> {
    ...
    await callTelegramApi(token, 'setMyCommands', {
      commands: [
        ...
        { command: 'sessions', description: '查看本地/Codex CLI会话（可直接恢复）' },
        { command: 'status', description: '查看当前状态' },
        { command: 'permission', description: '权限模式：ask / full / auto-review / status / 面板' },
        { command: 'mode', description: '切换模式：plan / code / ask' },
        { command: 'model', description: '查看或切换模型' },
        { command: 'import', description: '导入外部 Codex CLI 会话' },
        { command: 'resume', description: '恢复会话（支持会话ID，自动切目录）' },

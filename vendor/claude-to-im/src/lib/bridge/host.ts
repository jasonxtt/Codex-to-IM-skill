...
import type { ChannelBinding, ChannelType, PermissionProfile } from './types.js';
...
/** Approval policy values supported by Codex-style runtimes. */
export type StreamApprovalPolicy = 'never' | 'on-request' | 'on-failure' | 'untrusted';

/** Sandbox mode values supported by Codex-style runtimes. */
export type StreamSandboxMode = 'read-only' | 'workspace-write' | 'danger-full-access';
...
export interface StreamChatParams {
  ...
  /**
   * Optional runtime controls for hosts that support Codex-style thread options.
   * Hosts that do not support these fields can safely ignore them.
   */
